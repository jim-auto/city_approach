...
1. ~~Improve player/NPC animation.~~ **Done** (2026-04-19): `characters.js` generates 32×32 sprites on a `<canvas>` and registers them as Phaser textures (`player-0/1`, `npc-0/1`). Two render modes — `pixel` (default chibi pixel art, with {base, shadow, highlight} ramps per material, 3×3 sparkly eyes, smile, blush) and `icon` (emoji rendered via Canvas `fillText`). The mode is persisted in `localStorage` via `storage.js#getSpriteMode` and toggled from a TitleScene button. A 2-frame `player-walk` / `npc-walk` animation plays when the entity moves, and `flipX` handles left/right facing.
